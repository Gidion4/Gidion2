...
export function createProposal(from, title, description, options) {
    const proposal = {
...
export function castVote(proposalId, agent, option) {
    const vote = {
        agentId: agent.id,
        option,
...
export function calculateVoteResult(proposalId) {
    const key = `votes:${proposalId}`;
    const votes = cmGet(key) || [];
    const tally = {};
...
// Yhteistyötehtävä
// -----------------------------
export function assignCollaborativeTask(proposalId, task) {
