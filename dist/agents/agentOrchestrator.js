...
import { cmSet } from "./agentMemory.js";
// -----------------------------
// Agentin valinta tehtävään
// -----------------------------
export function selectAgentForTask(task) {
    const agents = Object.values(agentRegistry);
    if (agents.length === 0)
        return null;
    // Valitaan agentti roolin ja prioriteetin perusteella
    const candidates = agents.filter(a => task.requiredRole ? a.role === task.requiredRole : true);
...
// -----------------------------
export function createTeamForTask(task) {
    const agents = Object.values(agentRegistry);
...
// -----------------------------
export function startCollaborativeProcess(task) {
    const agents = Object.values(agentRegistry);
    if (agents.length === 0) {
        return { ok: false, message: "No agents registered." };
    }
    // 1) Luodaan yhteistyöehdotus
    const proposal = createProposal(agents[0], // ensimmäinen agentti aloittaa prosessin
    "Collaborative Task", "Determine the best strategy for the given task.", ["strategy-a", "strategy-b", "strategy-c"]);
