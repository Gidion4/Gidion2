...
export async function runInspection(ctx) {
    ...
    const prompt = `
Olet arkkitehtuuri-inspektori Gidion-järjestelmälle.

Tavoite: ${ctx.goal}

Sinulle annetaan otos Gidionin nykyisestä koodista (TypeScript, Node.js, ts-node, CommonJS-moduulit).
Analysoi:
...
Anna:
1) Ytimekäs yleiskuvaus nykytilasta
2) Lista tärkeimmistä parannuskohteista (bullet-lista)
3) Konkreettiset ehdotukset, miten Gidion voisi kehittää itseään (itse-evoluutio)
