...
import { EvolutionContext } from "./evolutionContext.js";
import { EvolutionIssue } from "./evolutionRules.js";
...
export async function runInSandbox(
...
): Promise<{ patches: EvolutionPatch[]; sandboxPath: string }> {
  ...
  const issueBlock =
    ...
    issues
      .map(
        (i) =>
          `// [${i.severity.toUpperCase()}] ${i.id}: ${i.message}${
            i.line ? " (line " + i.line + ")" : ""
          }`
      )
      .join("\n") +
  ...
  const patchBlock =
    ...
    patches
      .map(
        (p, idx) =>
          `// Patch ${idx + 1} for issue ${p.issueId}:\n//   ${p.description}\n//   Hint: ${p.hint}\n`
      )
      .join("\n") +
    "\n";
