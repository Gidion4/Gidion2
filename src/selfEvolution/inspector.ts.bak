...
function readAllFiles(dir: string, list: string[] = []): string[] {
...
export function inspectCodebase(rootDir: string): InspectionResult {
  const files = readAllFiles(rootDir);
  const issues: InspectionResult["issues"] = [];
