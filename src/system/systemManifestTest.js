// ------------------------------------------------------------
// GIDION LEVEL 4 — SYSTEM MANIFEST TEST v1 (ESM-COMPATIBLE)
// ------------------------------------------------------------
...
async function runTests() {
    ...
    for (const entry of systemManifest) {
        ...
        assert(fs.existsSync(fullPath), `Manifest entry '${entry.name}' missing at path: ${entry.path}`);
    }
    // --- Test 4: Kategoriat ovat validit ---
    const validCategories = ["core", "test", "agent", "system", "bootstrap"];
    for (const entry of systemManifest) {
        assert(validCategories.includes(entry.category), `Invalid category '${entry.category}' for entry '${entry.name}'`);
    }
    console.log("✔ All SystemManifest tests passed.");
    process.exit(0);
}
// ------------------------------------------------------------
// CLI ENTRYPOINT (ESM)
