// ------------------------------------------------------------
// GIDION LEVEL 6 â€” OPTIMIZATION ADVISOR TEST v1 (ESM-COMPATIBLE)
// ------------------------------------------------------------
...
async function runTests() {
  ...
  assert(
    typeof advice.metrics.health.requiresManualReview === "boolean",
  ...
  assert(
    typeof advice.metrics.selfHealing.plannedActions === "number",
    "metrics.selfHealing.plannedActions invalid"
  );
  assert(
    typeof advice.metrics.selfHealing.executedDryRunActions === "number",
    "metrics.selfHealing.executedDryRunActions invalid"
  );
  assert(
    typeof advice.metrics.selfHealing.dryRunAllSuccessful === "boolean",
  ...
  if (advice.advice.length > 0) {
    for (const item of advice.advice) {
      ...
      assert(
        ...
        "advice.severity invalid"
      );
    }
  }

  console.log("âœ” All OptimizationAdvisor tests passed.");
  process.exit(0);
}
