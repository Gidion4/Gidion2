// ------------------------------------------------------------
// GIDION LEVEL 6 — OPTIMIZATION DASHBOARD DATA TEST v2 (ESM-COMPATIBLE)
// ------------------------------------------------------------
import * as fs from "node:fs";
import * as path from "node:path";
import { fileURLToPath } from "node:url";
import { getOptimizationDashboardData } from "./systemOptimizationDashboardData.ts";
...
async function runTests() {
    ...
    assert(typeof data.metrics.health.requiresManualReview === "boolean", "metrics.health.requiresManualReview invalid");
    // selfHealing
    assert(typeof data.metrics.selfHealing === "object", "metrics.selfHealing invalid");
    assert(typeof data.metrics.selfHealing.plannedActions === "number", "metrics.selfHealing.plannedActions invalid");
    assert(typeof data.metrics.selfHealing.executedDryRunActions === "number", "metrics.selfHealing.executedDryRunActions invalid");
    assert(typeof data.metrics.selfHealing.dryRunAllSuccessful === "boolean", "metrics.selfHealing.dryRunAllSuccessful invalid");
    // repair
    ...
    for (const a of data.advice) {
        ...
        assert(a.recommendedAction === null || typeof a.recommendedAction === "string", "advice.recommendedAction invalid");
    ...
    console.log("✔ All OptimizationDashboardData tests passed.");
    process.exit(0);
}
// ------------------------------------------------------------
// CLI ENTRYPOINT (ESM)
