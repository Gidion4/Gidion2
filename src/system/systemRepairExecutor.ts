// ------------------------------------------------------------
// GIDION ULTRAHYBRID LEVEL 5 â€” REPAIR EXECUTOR v2 (ESM-COMPATIBLE)
// ------------------------------------------------------------
// TÃ¤mÃ¤ moduuli suorittaa Plannerin tuottamat korjaustoimet
// kÃ¤yttÃ¤en RepairActionLibraryn sallittuja toimintoja.
//
...
import { repairActions, RepairActionResult } from "./systemRepairActionLibrary.js";

export interface RepairStepExecution {
  actionKey: string;
  success: boolean;
  message: string;
}
...
export interface RepairExecutionReport {
  timestamp: string;
  steps: RepairStepExecution[];
  allSuccessful: boolean;
}

export async function executeRepairs(): Promise<RepairExecutionReport> {
  const plan = await generateHealingPlan();
  const steps: RepairStepExecution[] = [];
  ...
  for (const action of plan.actions) {
    ...
    const result: RepairActionResult = await repairFn();
...
// ------------------------------------------------------------
// Issue â†’ RepairActionLibrary -mapping
// ------------------------------------------------------------

function mapIssueToRepairAction(issueKey: string): string | null {
  switch (issueKey) {
    case "kernel":
...
if (import.meta.url === `file://${process.argv[1]}`) {
  executeRepairs().then((report) => {
    console.log("Gidion UltraHybrid Level 5 â€” Repair Executor");
