// ------------------------------------------------------------
// GIDION LEVEL 3 — COMMAND CONSOLE TEST HARNESS v1
// ------------------------------------------------------------
// Tämä tiedosto testaa koko komentoriviketjun:
...
//
// Tämä simuloi komentorivin käyttöä ilman oikeaa CLI:tä.
// ------------------------------------------------------------
import { handleConsoleInput } from "./commandConsole";
async function run() {
    console.log("=== GIDION LEVEL 3 — COMMAND CONSOLE TEST ===");
    // ------------------------------------------------------------
    // LUODAAN TAVOITTEET KOMENTORIVIN KAUTTA
    // ------------------------------------------------------------
    const c1 = await handleConsoleInput("tee testi pipeline");
    const c2 = await handleConsoleInput("tee järjestelmä joka analysoi videoita url linkin kautta näkemällä koko videon, ja tekee sen pohjalta toimenpiteet alusta loppuun halutulla tavalla");
    const c3 = await handleConsoleInput("tee raportti tehtävistä, jotka perustuvat gidionin kehittämiseen ja päivittämiseen");
